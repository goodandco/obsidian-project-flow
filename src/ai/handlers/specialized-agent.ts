...
export async function delegateToProjectAssistant(
  ...
  if (!resolved) {
    throw new Error(
      `Could not resolve project based on reference: ${JSON.stringify(args.projectRef)}`,
    );
  }
  ...
  const specializedTools = createSpecializedToolRegistry(
    plugin,
    projectTypeId,
    { id: resolved.entry.projectId },
    resolved.record,
  );
